...
export default function Header() {
  ...
  return (
    ...
    <div className="welcome hidden w-[100%] md:w-[80%] bg-white fixed top-0 left-0 md:left-[19%] font-manrope px-5 mx-[1%] py-2 text-font-color md:flex flex-row justify-between items-center z-50">    
      <div>
          <h1 className='text-[24px] font-semibold'>Welcome Aravind,</h1>
      ...
      <div className='flex flex-row gap-10 justify-center items-center'>
        ...
        <a className='flex-row gap-2 justify-center items-center cursor-pointer hidden md:flex'>
          <img className=' h-[40px] md:h-[50px] rounded-full' onClick={() => setPofile(!pofile)} src={facultyProfile} alt="facultyProfile" />
    ...
    <div className=' md:hidden block absolute top-[10%] m-5'>
      <h1 className='font-manrope font-semibold text-[24px]'>Welcome Aravind,</h1>
    ...
    `}> <ul className='w-[100%] flex justify-center px-4  flex-col gap-5 font-manrope'>
              ...
              <NavLink onClick={() => setHamburger(false)}  to="/faculty-assignments" className={({isActive}) => `flex flex-row gap-3 ml-2 px-3 py-3  hover:font-semibold hover:text-accent  transition-all duration-300 ease-in-out ${isActive ? "text-accent font-manrope font-bold bg-secondary-accent w-[90%] rounded-[5px]" : "text-gray-87"}`}>
              Assignments</NavLink>
              ...
              Settings</NavLink>
              <Link className='font-montserrat text-[16px] ml-2 px-3 font-semibold hover:font-semibold text-red-600' to="/faculty-login">Logout</Link>
            </ul>
      </div>
  {/* NOTIFICATIONS CARD */}
      {notification ?
      ...
      {pofile ?
        <div ref={profileRef} className=" font-manrope flex flex-col gap-3 gap px-5 py-5 bg-gray-100 shadow-md w-[350px] absolute top-[11%] right-[2%] rounded-[10px] z-50">
          <span className='flex flex-row gap-5 items-center px-2 py-2 bg-secondary-accent rounded-[10px]'>
            <img className='h-[50px] rounded-full' src={facultyProfile} alt="facultyProfile" />
            <span>
              <h4 className='text-[20px] font-semibold font-manrope'>Aravind</h4>
              <p className='font-montserrat font-regular'>2300EPID</p>
            </span>
          </span>
            ...
            <Link className='font-montserrat text-[16px] font-medium hover:font-semibold text-red-600' to="/faculty-login">Logout</Link>
        </div> : null
      }
    </>
  );
